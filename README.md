...
2.  執行腳本，傳入檔案路徑，其中 `-np` 參數是可選的：

    ```bash
    python create_sankey.py [您的 Excel 檔案路徑] -np [您要的百分比]
    ```
...
3.  輸出的 HTML 檔名會根據您的輸入檔名自動產生 (例如: `A0226_112_sankey_np5.0.html`)。

您可以直接在任何網頁瀏覽器中打開這個 HTML 檔案，以查看您的互動式 Sankey 圖表。

```
Pyecharts Sankey 圖表已成功生成！
檔案已儲存為: /.../workspace/result/A0226_112_sankey_np5.0.html
```